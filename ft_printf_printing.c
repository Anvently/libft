...
static int	print_null_byte(int fd, t_field *field, char *str)
{
	char	c;
	size_t	len_str;
	int		ret;
...
int	print_field(int fd, t_field *field)
{
	...
	if (field->type == '\0')
	{
		ft_putstr_fd((char *) field->value, fd);
	...
	if (field->type == 'c' && (unsigned long) field->value == 0)
		nbr_chars += print_null_byte(fd, field, str);
	else
		ft_putstr_fd(str, fd);
	free (str);
	return (nbr_chars);
...
int	print_fields(int fd, t_list *fields)
{
	...
	while (fields)
	{
		ret = print_field(fd, (t_field *) fields->content);
		if (ret < 0)
			return (-1);
		nbr_chars += ret;
		fields = fields->next;
